...
def solution(tickets) : 
    ...
    for k in graph.keys() : 
        ...
        graph[k] = sorted(graph[k], key = lambda x : (1 if 'ICN' in graph[x] else 0, 1 if k in graph[x] else 0) , reverse=True)
    def dfs(v, visited = []) : 
        visited.append(v)
        for node in graph[v] : 
            graph[v].remove(node)
...
answer = solution([['ICN','BOO'], ['ICN', 'COO'], ['COO', 'DOO'], ['DOO', 'COO'], ['BOO', 'DOO'], ['DOO', 'BOO'], ['BOO', 'ICN'], ['COO', 'BOO']])
# 조금 더 일반적인 원리를 생각해보자.
